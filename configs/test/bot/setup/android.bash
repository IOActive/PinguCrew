...
function run_bot () {
  ...
  cp -r $INSTALL_DIRECTORY/fuzzBot $bot_directory/fuzzBot
  echo "Created bot directory $bot_directory."
  cd $bot_directory/fuzzBot

  # Wait for device and run bot indefinitely for this bot.
  while true; do
    $ADB_PATH/adb -s "$serial" wait-for-device

    echo "Running ClusterFuzz instance for bot $serial."
    OS_OVERRIDE="ANDROID" ANDROID_SERIAL="$serial" PATH="$PATH" NFS_ROOT="$NFS_ROOT" GOOGLE_APPLICATION_CREDENTIALS="$GOOGLE_APPLICATION_CREDENTIALS" ROOT_DIR="$bot_directory/clusterfuzz" PYTHONPATH="$PYTHONPATH" GSUTIL_PATH="$GSUTIL_PATH" BOT_NAME="android-$(hostname)-$serial" HTTP_PORT_1="$((device_index+8000))" HTTP_PORT_2="$((device_index+8080))" python $bot_directory/fuzzBot/src/python/clussterfuzz/startup/run.py || true
