...
from src.frontend.handlers.InMemoryZip import InMemoryZip
...
def add_job():
    ...
    if flask.request.method == 'GET':
        engines = [x['name'] for x in current_app.config['mutation_engines']]
        fuzzers = [x['name'] for x in current_app.config['fuzzers']]
        verifiers = [x['name'] for x in current_app.config['verifiers']]
        ...
        if data.get('fuzzer') == "afl" or "syzkaller" or "libFuzzer":
            engine = 'external'
        ...
        new_job = Job(name=data.get('name'),
                      ...
                      maximum_samples=current_app.config['maximum_samples'],
                      archived=False,
                      enabled=True,
